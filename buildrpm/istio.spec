...
{{{- if semverCompare ">=1.15.1" $version }}}
########### install-cni ###############
%package install-cni
...
{{{- else }}}
make USE_LOCAL_PROXY=1 DEBUG=0 %{binaries} && echo "OK"
{{{- end }}}
...
install %{_builddir}/%{istio_go_src}/tools/packaging/common/gcp_envoy_bootstrap.json %{istio_pilot_artifact_build_dir}
{{{- end }}}
...
{{{- if semverCompare "<1.24.0" $version }}}
%{istio_pilot_artifact_file_dir}/gcp_envoy_bootstrap.json
{{{- end }}}
